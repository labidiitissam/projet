...
const TableReservationUser = ({ columns, title, data }) => {
    const deleteReservation = async (id) => {
        let user = localStorage.getItem("user");
        // Show an alert message to confirm the action
        const userConfirmed = window.confirm('Voulez-vous vraiment annuler cette réservation ?');

        // If the user confirms the action, proceed with the API call
        if (userConfirmed) {
            try {
                // Make an API call using Axios to your endpoint
                const response = await axios.put(`http://localhost:5000/api/reservation/cancel/${id}`, {}, {
                    headers: {
...
export default TableReservationUser;